...
<div class="wrap">
	...
	<div class="membership_input_wrap">
		<form name="frmJoin" id="insJoin_form" action="doIJoin.do" method="post" >
			...
			<div  id="membership">
				<div class="step2">
			        ...
			        <!-- ok , error 클레스 추가에 따라 텍스트 출력 및 스타일 변경됨-->
			            <dl>
			                <dt>아이디</dt>
			               <dd><input type="text" id="insId" name="insId" maxlength="25" placeholder="" autocomplete="off" title="아이디" /><span></span></dd>
	        ...
	        <div class="terms_wrap">
	            ...
	            <ul>
                    ...
                    <li>
                        <input type="checkbox" id="terms_agree3" class="agree_chk" autocomplete="off">
                        <label for="terms_agree3" ><em><i></i></em><span>개인정보 처리 위탁 동의 </span> <i>(필수)</i></label>
	                    <button type="button" id="btn3">보기</button>
                    </li>
                    <li>
	                    <input type="checkbox" id="terms_agree4" class="agree_chk" autocomplete="off">
	                    <label for="terms_agree4"><em><i></i></em><span>개인정보 수집 및 이용 동의 </span> <i>(선택)</i></label>
	                    <button type="button" id="btn4">보기</button>
	                </li>
	            </ul>
	        </div>
                    ...
                    <!--여기서부터-->
			        <div>
					    <button type="submit" id="submit" class="btn btn_type1 clr" data-btn-submit style="width: 400px; background-color: limegreen;">회원가입</button>
					</div>
					</form>
					</div>
				</div>
...
window.onload = function() {
    // '전체 동의' 체크박스
    var totalAgree = document.getElementById('terms_agree');
    
    // 개별 동의 체크박스 목록
    var agreeChks = document.querySelectorAll('.agree_chk');
    
    // '전체 동의' 체크박스 클릭 이벤트 핸들러
    totalAgree.addEventListener('change', function() {
        agreeChks.forEach(function(chk) {
            chk.checked = totalAgree.checked;
        });
    });
    ...
    agreeChks.forEach(function(chk) {
        chk.addEventListener('change', function() {
            ...
            var allChecked = Array.from(agreeChks).every(function(chk) {
                return chk.checked;
            });
            
            // '전체 동의' 상태 업데이트
            totalAgree.checked = allChecked;
        });
    });
};

function previewImage() {
    var fileInput = document.getElementById('insProfile');
